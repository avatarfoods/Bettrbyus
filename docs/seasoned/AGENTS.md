...
- The main checkout stays on `seasoned` at all times, kept current with `git pull`. Never commit, build, run servers, or test there. New worktrees are cut from the main checkout's HEAD (`worktree.baseRef` is `head`), so an outdated main checkout means an outdated lane. `scripts/lane.sh up` corrects that: it resets a lane without commits of its own onto the latest `origin/seasoned`, rebases one that already has commits, and warns when the rebase conflicts. In that case, rebase by hand before opening the pull request.
