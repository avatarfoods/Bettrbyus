...
- **`.worktreeinclude`** in the project root lists gitignored files Claude Code copies into every new worktree. It holds `.env.local` and `supabase/seed.sql`, so a fresh worktree already has its environment and, if the main checkout has one, the company data dump ready to load by hand (`[db.seed]` is disabled in `supabase/config.toml`, so nothing loads it automatically). `up` also copies both itself in case the worktree was made another way.
...
- `up` is idempotent: re-running reuses the port and the Supabase data, restarts the stack if it was stopped, and applies any migrations added since (`supabase migration up`). To start over, run `supabase db reset` inside the worktree, then `up` again to recreate the admin user, and reload any data dump by hand.
- The first `supabase start` in a lane replays every file in `supabase/migrations/`, exactly as in `INSTALL.md` step 8; the seed step is disabled there, so a lane starts with empty tables until you load a dump by hand. Docker images are shared between stacks.
